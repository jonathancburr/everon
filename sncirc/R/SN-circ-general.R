...
sncirc <- function(data, period.len=96, dist="Normal meanvar",max.cpts=5,minseglen=1,pen.val=3*log(length(data)),cost="Likelihood",pgcpt.method=FALSE){
...
sncirc.norm <- function(data,period.len=96,max.cpts=5,minseglen=1,pen.val=0,dist="Normal meanvar",all.seg){
  ...
  return(list(cps=apply(cps.M,1,sort,na.last=TRUE),op.ncpts=op.ncps,op.cpt.loc=cpts,op.like=op.like, like.M=like.M,
...
withinPeriod_plot <- function(data, period.len=96,test.stats='Normal meanvar',title='',param.lines=FALSE,cpts=c(period.len),params2plot="Mean",circData=TRUE){
  # assume data comes in two forms, matrix or vector.
  # vector: just the data. matrix/dataframe: two columns, first is data
  if(is.null(dim(data))){
    n = length(data)
  ...
  plot(data2plot, xlim=c(0,period.len+1), ylim=c(ylim1,ylim2),xlab='Fixed period',ylab=ylabel,main=title,pch=16)
  if(param.lines==TRUE){
    params = param.ests(data2plot, period.len, params2est=params2plot, cpts, circdata=FALSE)
    cpts = c(0,cpts)
    for(i in 1:(length(cpts)-1)){
      segments(x0=cpts[i],x1=cpts[i+1],y0=params[i],y1=params[i],col='red',lwd=2)
    }
    if(cpts[length(cpts)]!=period.len){segments(x0=cpts[length(cpts)],x1=period.len,y0=params[1],y1=params[1],col='red',lwd=2)}
  }
}
