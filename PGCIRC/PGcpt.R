...
pgcpt = function(data, period.len=12, minseglen.periodic=1, minseglen.global=2*period.len,
                 ...
                 penalty.periodic=3, penalty.global=max.periodic.cpts*log(length(data)), cost="Likelihood",
                 dist="Normal meanvar", restrict=FALSE, circData=TRUE, ...
){
  # period.len: the total number of observations within a fixed period, i.e. the length of the fixed period
  # minseglen.periodic: the minimum segment length at the periodic level (between periodic changepoints)
  # minseglen.global: minimum segment length between global changepoints
  ## min value is 2*period.len as this is the lowest number of periods to actually observe any periodic behaviour
  # method.periodic: periodic level changepoint search methodperiodic level changepoint search method
  # method.global: global level changepoint search methodperiodic level changepoint search method
  # max.periodic.cpts: SNcirc requires a maximum number of periodic changepoints to search over
  # penalty.periodic: penalty value for the periodic changepoint method. Assumed to be SIC, p*log(n), and the user can specify p
  # penalty.global: penalty value for the global changepoint method. Default to max.periodic.cpts*log(n).
  # cost: The cost function you are using, default to the Likelihood
  # dist: The assumed distribution of the data (and the type of change you want to look for)
  # restrict: whether you are restricting the global.method.
  ## TRUE==global.cpt.method runs on the start of every period (instead of every single time point)
  ## FALSE==global.cpt.method runs on every single time point.
  # circData: has the data been put into circular time, e.g. does it have a circular time index? 
  ## TRUE==the data needs to have a circular time index added to it (and so the code below will do it)
  ## FALSE==the data already has a circular time index column and is ready to be passed through the pgcpt algorithm
...
pgcpt.PELT = function(data, period.len=12, minseglen.periodic=1, minseglen.global=2*period.len,
  ...
  dist.list = c("Normal mean","Normal meanvar","Bernoulli", "Poisson") #list of distributions that has been coded
...
pgcpt.PELT.SNcirc <- function(data,period.len=12,minseglen.periodic=1, minseglen.global=2*period.len,max.periodic.cpts=5, 
  ...
  for(i in minseglen.global:(2*minseglen.global-1)){
    ...
    result = sncirc(data[1:i,],max.cpts=max.periodic.cpts,pen.val=penalty.periodic*log(i),
                    period.len=period.len, minseglen=minseglen.periodic,dist=dist,circData=FALSE)
    
    lastchangelike[i+1] = result$sncirc.results$op.like + penalty.global # the likelihood, incl. pen, for data from 1:i, choosing the optimal number of cpts
  ...
  for(tstar in (2*minseglen.global):n){
    ...
    for(i in 1:length(tmpt)){
      ...
      tmpcp[[i]] = result
      tmplike[i] = lastchangelike[tmpt[i]+1] + result$sncirc.results$op.like + penalty.global #this penalty is penalising a cpt in the macro-level
    }
  ...
  while(last!=0){ # macro cpts. cpp=micro cpts
    ...
    for (i in 1:ceiling((last.old-last)/period.len)){
      tmp=last+(i-1)*period.len + as.numeric(cps[[last.old+1]]$sncirc.results$op.cpt.loc) # getting the within-period cpts on the original scale - note that the start of the period is the across period cpt so it unlikely to be the same for each segment
  ...
  cpp.opt = list()
  for(i in 1:length(cpp)){
    cpp.opt[[i]] = cpp[[i]]$sncirc.results$op.cpt.loc
  }
