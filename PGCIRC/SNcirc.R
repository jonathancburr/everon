sncirc <- function(data, period.len=96, dist="Normal meanvar",max.cpts=5,minseglen=1,pen.val=3*log(length(data)),cost="Likelihood",circData=TRUE){
  ...
  dist.list = c("Normal mean","Normal meanvar","Bernoulli", "Poisson") #list of distributions that can be done
  ...
  if(dist=="Normal mean"){
    ...
    out = sncirc.pois(data=data.input,period.len=period.len,max.cpts=max.cpts,minseglen=minseglen,pen.val=pen.val,dist="Poisson")
  } else{
    stop(paste0("The only distributions available as of now are: ", paste(dist.list,collapse=', ')))
  }
...
# function to run SNcirc with normal distribution
sncirc.pois <- function(data,period.len=96,max.cpts=5,minseglen=1,pen.val=0,dist="Poisson",all.seg){
  if(missing(all.seg)){all.seg = sncirc.allseg.pois(data,period.len)}
  N = period.len
  M = max.cpts
...
# function to calculate all.seg with Poisson distribution
sncirc.allseg.pois <- function(data,period.len=96){
  #do the subset stuff before all.seg to reduce computational time
  N=period.len
  
  subset.mat = matrix(0,ncol=2,nrow=period.len) #col1=len, col2=sumx
  for(t in 1:N){ #go through for each within-period time index
    dat = subset(data,data[,1]==t)
    len = nrow(dat); sumx = sum(dat[,2])
    subset.mat[t,] = c(len,sumx)
  ...
  for(i in 1:N){
    ...
    for(j in i:jend){
      ...
      sumx = sumx + subset.mat[j.circ,2]
      if(sumx==0){
        all.seg[i,j.circ] = 0 # avoid the problem if sumx=0 and having to deal with log(sumx)
      } else{
        all.seg[i,j.circ]=-sumx*(1+log(len)-log(sumx))
      }
    }
  }
...
withinPeriod_plot <- function(data, period.len=96,test.stats='Normal meanvar',title='',param.lines=FALSE,cpts=c(period.len),params2plot="Mean",circData=TRUE,param.est.col='red',defaultAxes=TRUE){
  ...
  # test.stats options: norm, bern, pois
